<template>
  <div :class="isSidebarOpen || isFixedSidebar ? 'min-h-screen flex relative lg:static bg-[#f8fafc]' : 'min-h-screen flex relative lg:static bg-[#f8fafc]'">
    <div id="app-sidebar-10"
      ...
      :class="isFixedSidebar ? 'w-[300px]' : 'w-0'"
      ...
      <div class="flex h-full">
        <div class="flex flex-col h-full bg-[#059669] flex-shrink-0 select-none">
            ...
            <ul class="list-none py-4 pl-2 pr-0 m-0">
              <li class="mb-2" v-for="(item, index) in menuItems" :key="index">
                <a class="rounded-l-full flex items-center cursor-pointer py-4 pl-0 pr-2 justify-center hover:bg-[#10B981] text-[#FFFFFF] duration-150 transition-colors"
                  :class="{ 'bg-[#10B981]': activeTab2 === index }"
          ...
          <div class="mt-auto">
            ...
            <a class="m-4 flex items-center cursor-pointer p-2 justify-center hover:bg-[#10B981] rounded-border text-[#cbd5e1] hover:text-[#ffffff] duration-150 transition-colors">
        ...
        <div class="flex flex-col bg-[#10B981] w-[230px] p-6 overflow-y-auto flex-shrink-0 grow md:grow-0"
     :class="{ 'hidden': !isSidebarOpen && !isFixedSidebar, 'lg:block': isFixedSidebar || isSidebarOpen }">
  <div class="justify-end mb-4 flex">
    <div class="flex items-center gap-2">
      <Checkbox inputId="size_normal" name="size" :value="true" v-model="isFixedSidebar" @change="toggleSidebar" />
    </div>
  </div>
  <div class="rounded-border flex-auto">
    ...
    <div class="min-h-screen flex flex-col relative flex-auto" :class="isSidebarOpen || isFixedSidebar ? '' : 'w-[100%]'">
      <div class="h-[60px] flex justify-between lg:justify-start items-center px-8 bg-[#ffffff] border-b border-surface relative">
      </div>
...
// Toggle sidebar behavior based on checkbox state
const toggleSidebar = () => {
  debugger
    if (isFixedSidebar.value) {
        isSidebarOpen.value = true; // Keep sidebar open
    } else {
        isSidebarOpen.value = false; // Close sidebar
    }
};

// Open the sidebar on hover (if not fixed)
const openSidebar = () => {
    if (!isFixedSidebar.value) {
...
// Close the sidebar on mouse leave (if not fixed)
...
// Change the active tab when a menu item is clicked
...
watch(isFixedSidebar, (newValue) => {
    if (Array.isArray(newValue) && newValue.includes(true)) {
        isSidebarOpen.value = true; // Open sidebar if the array contains true
    } else {
        isSidebarOpen.value = false; 
